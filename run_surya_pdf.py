...
def process_single_pdf(pdf_path, output_dir, predictor):
    pdf_path = Path(pdf_path)
    if not pdf_path.exists():
        print(f"❌ Error: Không tìm thấy file {pdf_path}")
        return

    # Thư mục đầu ra cho riêng cuốn sách này
    ...
    for page_num in range(total_pages):
        ...
        # Chạy dự đoán bằng Surya
        results = predictor([pil_img], full_page=True)
        
        boxes = []
        page_text_lines = []
        
        for blk in results[0].blocks:
            if blk.skipped or getattr(blk, 'error', False) or not blk.html:
                continue
            
            # Xử lý xoá thẻ HTML, giữ lại \n
            text = re.sub(r'<br\s*/?>', '\n', blk.html)
            text = re.sub(r'<[^>]+>', '', text).strip()
            if not text:
                continue
                
            poly = np.array(blk.polygon, float)
            x1, y1 = poly.min(0)
            x2, y2 = poly.max(0)
...
def main():
    parser = argparse.ArgumentParser(description="Script chạy Surya OCR cho hàng loạt file PDF")
    ...
    # Gom danh sách các file PDF cần chạy
    pdf_files = []
    ...
    # Khởi tạo model MỘT LẦN duy nhất để tối ưu thời gian và VRAM GPU cho tất cả các file
    print(f"🚀 Khởi tạo model Surya OCR (sẽ tự động dùng GPU nếu khả dụng)...")
    from surya.recognition import RecognitionPredictor
    predictor = RecognitionPredictor()

    for pdf in pdf_files:
        process_single_pdf(pdf, args.out, predictor)
